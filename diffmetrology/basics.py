    ...
    def __str__(self):
        ...
        for key, val in vars(self).items():
            if val.__class__.__name__ in ('list', 'tuple'):
                ...
                pass    # ignore outputs for Hash tables
            elif key == key.upper() and len(key) > 5:
                pass    # ignore all upper-case variables > 5 chars (constants)
            else:
...
class Ray(PrettyPrinter):
    def __init__(self, o, d, wavelength, device=torch.device('cpu')):
        self.o = o  # ray origin
        self.d = d  # ray direction (normalized)
...
def generate_test_rays():
    ...
    # d = np.array([0.02, -0.03, 1])
    # d = np.tile(d[None, None, ...], [*filmsize, 1])
...
if __name__ == "__main__":
    ...
    assert np.sum(np.abs(to_world.R.numpy() @ o_old + to_world.t.numpy() - o_new)) < 1e-15
